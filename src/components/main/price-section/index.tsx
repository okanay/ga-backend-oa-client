...
export const PriceSection = () => {
  ...
  const clipTop = useTransform(scrollYProgress, [0, 0.55, 1], [100, 100, 0]);
  const clipTemplate = useMotionTemplate`inset(0% 0% ${clipTop}% 0%)`;

  return (
    <section
      ref={sectionRef}
      className={"relative h-full max-h-[1000px] bg-flower-50 py-24"}
    >
      <div className={"flex flex-col items-center justify-center space-y-12"}>
        ...
        {/*Card*/}
        <m.div
          className={`relative z-30 h-[480px] w-[340px] space-y-6 rounded-lg 
        ...
        >
          <div className={"flex h-full w-full flex-col justify-between"}>
            ...
            <div className="w-full">
              <button className="h-[60px] w-full rounded-lg border border-secondary-950/10 bg-secondary-700 text-xl tracking-widest text-flower-50">
                Start Now
              </button>
            </div>
          </div>
      ...
      <m.div
        className="absolute bottom-0 left-0 z-20 hidden h-[350px] w-full bg-secondary-700 sm:block"
        style={{
